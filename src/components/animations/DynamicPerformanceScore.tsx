...
export function DynamicPerformanceScore() {
    ...
    useEffect(() => {
        ...
        const sequence = async () => {
            setCheckedIds([]);
            countValue.set(0);
            await framerAnimate(scope.current, { opacity: 1 }, { duration: 0 });
    ...
    return (
        <div ref={containerRef} className="flex flex-col md:flex-row gap-8 w-full group">
            ...
            <div ref={scope} className="w-full md:w-[55%] bg-cream rounded-2xl p-6 md:p-8 border border-charcoal/5 flex flex-col gap-4 relative overflow-hidden">
                {/* Custom SVG Cursor */}
                <motion.div
                    className="cursor-svg absolute z-50 text-charcoal pointer-events-none drop-shadow-lg"
                    initial={{ opacity: 0 }}
                ...
                <div className="flex items-center justify-between border-b border-charcoal/10 pb-4 mb-2">
                    <span className="text-xs font-bold text-charcoal-light tracking-widest uppercase">Habit Tracker</span>
                    ...
                    {mockHabits.map((habit) => {
                        ...
                        return (
                            <motion.div
                                id={`row-${habit.id}`}
                                key={habit.id}
                                className={`flex items-center justify-between p-3 rounded-xl border relative overflow-hidden transition-colors duration-300 ${isChecked ? 'bg-primary/5 border-primary/20 shadow-sm' : 'bg-white border-charcoal/5'}`}
                            >
                                <div className="flex items-center gap-4 relative z-10">
                                    <motion.div
                                        layout
                                        className={`size-5 shrink-0 rounded-full border-2 flex items-center justify-center transition-colors duration-300 ${isChecked ? 'border-primary' : 'border-charcoal/20'}`}
                                    >
                                        <motion.div
                                            initial={{ scale: 0 }}
                                    ...
                                    <span className={`text-sm font-medium transition-all duration-300 ${isChecked ? 'text-charcoal-light line-through' : 'text-charcoal'}`}>
                                        {habit.label}
                                    </span>
                                </div>

                                <div className="flex items-center gap-2 relative z-10">
                                    <span className={`text-[10px] font-bold px-2 py-1 rounded-[4px] uppercase tracking-wider transition-colors duration-300 ${isChecked ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-charcoal/5 text-charcoal-light border border-charcoal/10'}`}>
            ...
            <div className="w-full md:w-[45%] flex flex-col items-center justify-center gap-6 bg-white p-8 rounded-2xl relative border border-charcoal/5 overflow-hidden">
                {/* Progress Bars */}
                ...
                <div className="relative flex flex-col items-center">
                    <span className="text-xs text-charcoal-light font-bold tracking-widest uppercase mb-1">
                        Efficiency Score
                    </span>
                    <motion.div
                        className="font-serif italic text-[5rem] leading-none text-charcoal relative z-10 transition-all duration-300"
                        animate={{
                            color: checkedIds.length === 3 ? "#2D5A3D" : "#2A2A2A"
                        }}
                    >
